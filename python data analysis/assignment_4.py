...
def top_player_ids(info, statistics, formula, numplayers):
    ...
    for row in statistics:
        ...
        stat = formula(info, row)
        new_tuple = tuple((player_id, stat))
        datalist.append(new_tuple)

    datalist.sort(key=lambda pair: pair[1], reverse=True)
    for tuple_idx in datalist:
        if numplayers > 0:
...
def lookup_player_names(info, top_ids_and_stats):
    ...
    """
    new_list = []
    info_dict = read_csv_as_nested_dict(info['masterfile'], info['playerid'], info['separator'], 
                                        info['quote'])
    for idx in top_ids_and_stats:
        formatted_string = format(
            idx[1], '.3f') + " --- {0} {1}".format(info_dict[idx[0]][info['firstname']],
...
def compute_top_stats_year(info, formula, numplayers, year):
    ...
    """
    stat_list = read_csv_as_list_dict(info['battingfile'],
                                      info['separator'],
                                      info['quote']
                                      )
    filtered_by_year = filter_by_year(stat_list,
                                      year,
                                      info['yearid'])
...
def compute_top_stats_career(info, formula, numplayers):
    ...
    """
    stats = []
    aggregated_stats = aggregate_by_player_id(read_csv_as_list_dict(info['battingfile'], 
                                                                    info['separator'], 
                                                                    info['quote']),
                                              info['playerid'],
                                              info['battingfields'])
